...
<?php if (isset($_SESSION['email'])) : ?>
    ...
    <div class="flex justify-center">
        <div class="pr-16">
            ...
            <h3 class="border border-black p-2 rounded  text-blue-500"><?php echo $client[0]['name'] ?></h3>
            <h1 class="font-bold text-2xl">Address</h1>
            <h3 class="border border-black p-2 rounded  text-blue-500"><?php echo $client[0]['address'] ?></h3>
        </div>
        <div>
            <h1 class="font-bold text-2xl">Email</h1>
            <h3 class="border border-black p-2 rounded  text-blue-500"><?php echo $client[0]['email'] ?></h3>
            <h1 class="font-bold text-2xl">Role</h1>
            <h3 class="border border-black p-2 rounded  text-blue-500"><?php echo $client[0]['role'] ?></h3>
        </div>
        <div class="flex items-center ml-12">
            <form action="user-edit" method="post">
                <input type="hidden" name="id" value="<?php echo $client[0]['id']; ?>" />
                <input type="hidden" name="name" value="<?php echo $client[0]['name']; ?>" />
                <input type="hidden" name="address" value="<?php echo $client[0]['address']; ?>" />
                <input type="hidden" name="email" value="<?php echo $client[0]['email']; ?>" />
                <input type="hidden" name="password" value="<?php echo $client[0]['password']; ?>" />
